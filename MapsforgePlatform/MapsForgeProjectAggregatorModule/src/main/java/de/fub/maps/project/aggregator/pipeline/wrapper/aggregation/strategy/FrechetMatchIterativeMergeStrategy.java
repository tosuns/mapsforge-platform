 ...
 */
@NbBundle.Messages({
    "FrechetMatchIterativeMergeStrategy_Name=Frechet Merge Match Attraction Merge",
    "FrechetMatchIterativeMergeStrategy_Description=No description available",
    "FrechetMatchIterativeMergeStrategy_Settings_Name=Frechet Merge Match Attraction Merge Settings",
    "FrechetMatchIterativeMergeStrategy_Settings_Description=Parameters to configure this aggregations strategy",
    "FrechetMatchIterativeMergeStrategy_MaxLookahead_Name=Max Lookahead",
    "FrechetMatchIterativeMergeStrategy_MaxLookahead_Description=No description available",
    "FrechetMatchIterativeMergeStrategy_MaxPathDifference_Name=Max Path Difference",
    "FrechetMatchIterativeMergeStrategy_MaxPathDifference_Description=No description available",
    "FrechetMatchIterativeMergeStrategy_MaxInitDistance_Name=Max Init Distance",
    "FrechetMatchIterativeMergeStrategy_MaxInitDistance_Description=No description available",
    "FrechetMatchIterativeMergeStrategy_MergeHandler_Type_Name=MergeHandler Instance Type",
    "FrechetMatchIterativeMergeStrategy_MergeHandler_Type_Description=The instance that is responsible for merge the data points",
    "FrechetMatchIterativeMergeStrategy_TraceDistance_Type_Name=TraceDistance Instance Type",
    "FrechetMatchIterativeMergeStrategy_TraceDistance_Type_Description=The instance which is responsible fo tracing the distance of a segment"
})
@ServiceProvider(service = AggregationStrategy.class)
public class FrechetMatchIterativeMergeStrategy extends de.fub.agg2graph.agg.strategy.FrechetMatchIterativeMergeStrategy implements AggregationStrategy {

    private static final Logger LOG = Logger.getLogger(FrechetMatchIterativeMergeStrategy.class.getName());
    private static final String PROP_NAME_MAX_LOOKAHEAD = "FrechetIterative.max.lookahead";
    private static final String PROP_NAME_MAX_PATHDIFFERENCE = "FrechetIterative.max.pathdifference";
    private static final String PROP_NAME_MAX_INITDISTANCE = "FrechetIterative.max.initDistance";
    private static final String PROP_NAME_BASE_MERGEHANDLER_TYPE = "FrechetIterative.base.mergehandler.type";
    private static final String PROP_NAME_TRACE_DISTANCE_TYPE = "FrechetIterative.trace.distance.type";
                ...
                for (ProcessDescriptor descriptor : getAggregator().getAggregatorDescriptor().getPipeline().getList()) {
                    if (descriptor != null && AggregationProcess.class.getName().equals(descriptor.getJavaType())) {
                        for (PropertySection section : descriptor.getProperties().getSections()) {
                            if (FrechetMatchIterativeMergeStrategy.class.getName().equals(section.getId())) {
    ...
    private void reInit() {
        ...
        if (propertySection != null) {
            for (PropertySet propertySet : propertySection.getPropertySet()) {
                if (Bundle.FrechetMatchIterativeMergeStrategy_Settings_Name().equals(propertySet.getName())) {
    ...
    private PropertySection createDefaultDescriptor() {
        ...
        propertySet.setId(FrechetMatchIterativeMergeStrategy.class.getName());
        ...
        PropertySection section = new PropertySection(
                Bundle.FrechetMatchAttractionMergeStrategy_Settings_Name(),
                Bundle.FrechetMatchAttractionMergeStrategy_Settings_Description());
        section.setId(FrechetMatchIterativeMergeStrategy.class.getName());
    ...
    private static class StrategyNode extends AbstractNode {
        ...
        private final FrechetMatchIterativeMergeStrategy strategy;

        public StrategyNode(FrechetMatchIterativeMergeStrategy strategy) {
            super(Children.LEAF);
            ...
            if (strategy != null) {
                ...
                if (propertySection != null) {
                    for (final de.fub.maps.project.aggregator.xml.PropertySet propertySet : propertySection.getPropertySet()) {
                        if (FrechetMatchIterativeMergeStrategy.class.getName().equals(propertySet.getId())) {
                            ...
                            for (final de.fub.maps.project.aggregator.xml.Property property : properties) {
                                if (FrechetMatchIterativeMergeStrategy.PROP_NAME_MAX_LOOKAHEAD.equals(property.getId())) {
                                    set.put(new NodeProperty(property));
                                } else if (FrechetMatchIterativeMergeStrategy.PROP_NAME_MAX_PATHDIFFERENCE.equals(property.getId())) {
                                    set.put(new NodeProperty(property));
                                } else if (FrechetMatchIterativeMergeStrategy.PROP_NAME_MAX_INITDISTANCE.equals(property.getId())) {
                                    set.put(new NodeProperty(property));
                                } else if (FrechetMatchIterativeMergeStrategy.PROP_NAME_BASE_MERGEHANDLER_TYPE.equals(property.getId())) {
                                    ClassProperty classProperty = new MergeHandlerProperty(propertySection, property);
                                    set.put(classProperty);
                                } else if (FrechetMatchIterativeMergeStrategy.PROP_NAME_TRACE_DISTANCE_TYPE.equals(property.getId())) {
