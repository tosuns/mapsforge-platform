 ...
 */
@NbBundle.Messages({
    "AttractionForceMerge_Name=Attraction Force Merge Handler",
    "AttractionForceMerge_Description=The default merge handler implementation",
    "AttractionForceMerge_MaxPointGhostDistance_Name=Max Point Ghost Distance",
    "AttractionForceMerge_MaxPointGhostDistance_Description=No description available",
    "AttractionForceMerge_MaxLookahead_Name=Max Lookahead",
    "AttractionForceMerge_MaxLookahead_Description=No description available",
    "AttractionForceMerge_MinContinuationAngle_Name=Min Continuation Angle",
    "AttractionForceMerge_MinContinuationAngle_Description=No description available",
    "AttractionForceMerge_Setting_PropertySet_Name=Attraction Force Merge Handler Settings",
    "AttractionForceMerge_Setting_PropertySet_Description=Parameter to configure the merge handler instance."
})
@ServiceProvider(service = MergeHandler.class)
public class AttractionForceMerge extends de.fub.agg2graph.agg.strategy.AttractionForceMerge implements MergeHandler {

    private static final Logger LOG = Logger.getLogger(AttractionForceMerge.class.getName());
    private static final String PROP_NAME_MAX_POINT_GHOST_DISTANCE = "AttractionForceMerge.AttractionForceMerge.max.point.ghost.distance";
    private static final String PROP_NAME_MAX_LOOKAHEAD = "AttractionForceMerge.AttractionForceMerge.max.lookahead";
    private static final String PROP_NAME_MIN_CONTINUATION_ANGLE = "AttractionForceMerge.min.continuation.angle";
                    ...
                    if (descriptor != null
                        ...
                        for (PropertySection propertySection : sections) {
                            for (PropertySet set : propertySection.getPropertySet()) {
                                ...
                                if (AttractionForceMerge.class.getName().equals(set.getId())) {
                                    propertySet = set;
                                    break;
                                }
                            }
    ...
    private PropertySet createDefaultPropertySet() {
        ...
        PropertySet set = new PropertySet(
                Bundle.AttractionForceMerge_Setting_PropertySet_Name(),
                Bundle.AttractionForceMerge_Setting_PropertySet_Description());
        set.setId(AttractionForceMerge.class.getName());

        Property property = new Property();
        property.setId(PROP_NAME_MAX_LOOKAHEAD);
        property.setJavaType(Integer.class.getName());
        property.setName(Bundle.AttractionForceMerge_MaxLookahead_Name());
        property.setDescription(Bundle.AttractionForceMerge_MaxLookahead_Description());
        ...
        property.setName(Bundle.AttractionForceMerge_MaxPointGhostDistance_Name());
        property.setDescription(Bundle.AttractionForceMerge_MaxPointGhostDistance_Description());
        ...
        property.setJavaType(Double.class.getName());
        property.setName(Bundle.AttractionForceMerge_MinContinuationAngle_Name());
        property.setDescription(Bundle.AttractionForceMerge_MinContinuationAngle_Description());
    ...
    public Node getNodeDelegate() {
        if (nodeDelegate == null) {
            nodeDelegate = new MergeHandlerNode(AttractionForceMerge.this);
        }
        return nodeDelegate;
    }
    ...
    private static class MergeHandlerNode extends AbstractNode {
        ...
        private final AttractionForceMerge mergeHandler;

        public MergeHandlerNode(AttractionForceMerge mergeHandler) {
            super(Children.LEAF);
