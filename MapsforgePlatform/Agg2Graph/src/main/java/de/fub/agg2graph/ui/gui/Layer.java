    ...
    public boolean repaint(boolean force) {
        ...
        for (Object loopObject : loopCopyObjects) {
                ...
                if (visibleArea != null) {
                    ...
                    if (nodes != null) {
                        ...
                        for (AggConnection conn : conns) {
                            if (layerManager.renderWeight) {
                                lineRo = conn.isVisible() ? roVisible : roHidden;
            ...
            if (object instanceof Set<?>) { // Set<Intersection>
                ...
                intersectionRo.setStrokeBaseWidthFactor(7 + 17 / 6 * (layerManager.getUi().getMainPanel().getZoom() - 12));
            ...
            if (object instanceof RoadNetwork) {
                ...
                for (Road r : roadNetwork.roads) {
                    ...
                    for (int i = 1; i < nodes.size(); i++) {
                        drawLine(g2, nodes.get(i - 1), nodes.get(i), roInternal, 1, (i == nodes.size() - 1 && r.isOneWay()));
                    }
                }
        ...
        labelFont = labelFont.deriveFont((float) (32 - 3 * Math.log(drawnPointsCounter)));
        g2.setFont(labelFont);

        // actually draw
        Polygon arrowHead = new Polygon();
        arrowHead.addPoint((int) (-4 * layerManager.thicknessFactor), (int) (-8 * layerManager.thicknessFactor));
