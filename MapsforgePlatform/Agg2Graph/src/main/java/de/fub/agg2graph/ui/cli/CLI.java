...
public class CLI {
     ...
     * without UI.
     *
     * @param args list of filenames (search is conducted in folder cli), if
    ...
    public void run(String[] args) throws FileNotFoundException, IOException,
        ...
        for (File currentFile : targetFiles) {
            ...
            logger.info(MessageFormat.format("processing file {0}", file.getName()));
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if ("input".equals(t.name)) {
                    ...
                    aggFolder = (tokens.size() > tokenIndex + 1
                            ? new File(MessageFormat.format("test/agg/{0}", tokens.get(tokenIndex + 1).name))
                            : aggFolder);
                    aggFolder.mkdirs();
                    ...
                    for (File inputFile : inputFiles) {
                        logger.info(MessageFormat.format("processing file {0}", inputFile.getAbsolutePath()));
                        ...
                        if (segments == null) {
                            logger.severe(MessageFormat.format("Bad file: {0}", file));
                            continue;
                        }
                        List<GPSSegment> cleanSegments = new ArrayList<GPSSegment>(
                                0);
                        for (GPSSegment segment : segments) {
                            segment.addIDs(MessageFormat.format("I{0}", counter++));
                            // clean?
                            if (stepStorage
                    ...
                    logger.info(MessageFormat.format("Writing OSM data to {0}", outFile));
                    osmExport.export(stepStorage.getRoadNetwork(), new FileOutputStream(outFile));
                } else if ("loadagg".equals(t.name)) {
                    aggFolder = (tokens.size() > tokenIndex + 1 ? new File(
                            MessageFormat.format("test/agg/{0}", tokens.get(tokenIndex + 1).name))
                            : aggFolder);
                    makeAgg(aggFolder);
    ...
    public static void printParameters() {
        ...
        for (int i = 0; i < CalcThread.stepNames.length; i++) {
            sb.append("Parameters for ").append(CalcThread.stepNames[i]).append("\n");
    ...
    public void printStatistics(String baseFilename) throws IOException {
        File outputFile = new File(MessageFormat.format("{0}{1}{2}-{3}{4}.{5}", outputFolder.getAbsoluteFile(), File.separator, file.getName(), baseFilename.split("\\.")[0], tokens.size() > 1 ? "-" + tokens.get(1).name : "", baseFilename.split("\\.")[1]));
        ...
        for (String stat : stats.keySet()) {
            outputString = MessageFormat.format("{0}: {1}", stat, stats.get(stat));
